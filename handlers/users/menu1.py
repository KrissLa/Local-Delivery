...
from keyboards.inline.callback_datas import product_list_data, categories_data, \
    product_count_price_data, deliver_to_time_data, size_product_data
from keyboards.inline.inline_keyboards import generate_keyboard_with_categories, generate_keyboard_with_products, \
    generate_keyboard_with_count_and_prices, \
    one_more_product_markup, delivery_options_markup, order_cancel_or_back_markup, need_pass_markup, \
    deliver_to_time_markup, confirm_order_markup, generate_keyboard_with_sizes, \
    generate_keyboard_with_count_and_prices_for_size
    ...
    if len(product_info) == 10:
        ...
        await call.message.answer('Для заказа выберите размер.',
                                  reply_markup=await generate_keyboard_with_sizes(product_info['list_of_sizes'],
                                                                                  product_info['product_info']['product_category_id']))
        await Menu.WaitProductSize.set()


@dp.callback_query_handler(size_product_data.filter(), state=Menu.WaitProductSize)
async def get_product_size(call: CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    print(size_info)
    await call.message.answer("Укажите количество:",
                              reply_markup=await generate_keyboard_with_count_and_prices_for_size(size_info, product_id))
    state_name = await state.get_state()
    print(state_name)
    if state == 'Menu:WaitProductSizeBack':
        await Menu.WaitQuantityFromSize.set()
    else:
        await Menu.WaitQuantity.set()
...
async def get_quantity_more_than_6(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if quantity < 6:
            ...
            order_data = await state.get_data()
            order_price = int(order_data['order_detail']['product_price']) * quantity
...
async def set_pickup(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(f'Ваш заказ № {order_id}:\n'
                              ...
                              reply_markup=deliver_to_time_markup)
...
@dp.callback_query_handler(text='need_pass_true', state=Menu.WaitPass)
async def need_pass_true(call: CallbackQuery):
    """Пропуск заказан"""
    await call.message.edit_reply_markup()
    order_data = await db.get_last_user_order_detail(user_id=call.from_user.id)
    couriers_list = await db.get_couriers_list(order_data['order_location_id'])
...
@dp.callback_query_handler(text='need_pass_false', state=Menu.WaitPass)
async def need_pass_false(call: CallbackQuery):
...
async def get_time_of_delivery(call: CallbackQuery, callback_data: dict):
    ...
    delivery_to = datetime.now(timezone('Europe/Moscow')) + timedelta(minutes=min)
    order_data = await db.get_last_user_order_detail(user_id=call.from_user.id)
    await db.update_deliver_to(order_id=order_data['order_id'], time_value=delivery_to)
    ...
    if order_data['delivery_method'] == 'С доставкой':
        await call.message.answer(f'Ваш заказ № {order_data["order_id"]}:\n'
                                  ...
                                  f'Время доставки: {delivery_to.strftime("%H:%M")}\n'
                                  f'Сумма заказа - {order_data["order_price"]} руб.',
        ...
        await call.message.answer(f'Ваш заказ № {order_data["order_id"]}:\n'
                                  ...
                                  f'Приготовоить ко времени: {delivery_to.strftime("%H:%M")}\n'
                                  f'Сумма заказа - {order_data["order_price"]} руб.',
                                  reply_markup=confirm_order_markup)
    await Menu.WaitUserConfirmation.set()


@dp.callback_query_handler(text='confirm_order', state=Menu.WaitUserConfirmation)
async def user_confirm_order(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("Готово.\n"
                              ...
                              "Вы получите уведомление о готовности заказа.")
    await state.finish()
    # await db.
