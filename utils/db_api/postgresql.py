...
class Database:
    ...
    #     try:
    #         await self.pool.execute(
    #             "CREATE TYPE or_status AS ENUM ('Ожидание пользователя', 'Ожидание подтверждения продавца', "
    #             "'Подтвержден, готовится', 'Отклонен', 'Отменен пользователем', 'Заказ готов', "
...
AND is_local_object_available = true ORDER BY metro_id""")
    ...
    async def add_order(self,
        ...
        sql = """
        INSERT INTO orders (order_user_id, order_local_object_id, order_final_price,
         order_delivery_method, order_status) 
        VALUES ($1, $2, $3, $4, $5)"""
        done = False
        count = 0
        while not done:
    ...
    async def add_order_pickup(self,
        ...
        """Добавляем новый заказ в таблицу"""
        sql = """
        INSERT INTO orders (order_user_id, order_local_object_id, order_address,
         order_final_price, order_delivery_method, order_status) 
        VALUES ($1, $2, $3, $4, $5, $6)"""
        done = False
        count = 0
    ...
    async def get_user_address_data(self, user_id):
        """Получаем инфу о пользователе для создания заказа"""
        return await self.pool.fetchrow(
            f"""SELECT user_id, user_local_object_id, user_address, location_address, local_object_name
                FROM users 
                JOIN locations ON user_location_id=location_id
                JOIN local_objects ON local_object_id=user_local_object_id
                WHERE user_telegram_id = {user_id};"""
    ...
    async def get_last_order_id(self, user_id):
        ...
        return await self.pool.fetchval(
            ...
            f'WHERE order_user_id = (select user_id from users where user_telegram_id = {user_id}) '
            f'ORDER BY -order_id'
        )
    ...
    async def get_last_order_data(self, user_id):
        ...
        return await self.pool.fetchrow(
            f"""SELECT order_id, local_object_name, location_id, order_final_price, order_address
FROM orders 
...
ORDER BY -order_id"""
    ...
    async def get_last_order_big_data(self, user_id):
        ...
        return await self.pool.fetchrow(
            f"""SELECT order_id, local_object_name, location_id, order_final_price, order_address, 
order_delivery_method, order_pass_to_courier, order_deliver_through, order_status
    FROM orders 
    JOIN local_objects on local_object_id=order_local_object_id
    JOIN locations on local_object_location_id=location_id
    WHERE order_user_id = (select user_id from users where user_telegram_id = {user_id})
    ...
    async def get_order_data(self, order_id):
        """Получаем данные текущего заказа"""
        return await self.pool.fetchrow(
            f"""SELECT local_object_name, order_final_price, order_address, order_time_for_delivery, user_telegram_id
        FROM orders 
        JOIN local_objects on local_object_id=order_local_object_id
    ...
    async def get_active_order_data(self, order_id):
        """Получаем данные текущего заказа"""
        return await self.pool.fetchrow(
            f"""SELECT local_object_name, order_final_price, order_address, order_time_for_delivery, courier_telegram_id
        FROM orders 
        JOIN local_objects on local_object_id=order_local_object_id
    ...
    async def update_order_address(self, order_id, address):
        ...
        await self.pool.execute(
            f"""UPDATE orders SET order_address = '{address}' WHERE order_id = {order_id}"""
        )
    ...
    async def update_order_pass(self, order_id, pass_value):
        """Обновляем адрес доставки"""
        await self.pool.execute(
            f"""UPDATE orders SET order_pass_to_courier = '{pass_value}' WHERE order_id = {order_id}"""
        )
    ...
    async def update_deliver_to(self, order_id, time_value, now_time):
        ...
        await self.pool.execute(
            f"""UPDATE orders 
SET order_time_for_delivery = '{time_value}',
order_accepted_at = '{now_time}'
WHERE order_id = {order_id}"""
        )
...
WHERE order_id = {order_id}"""
    ...
    async def clear_empty_orders(self, user_id):
        """Очищаем корзину пользователя"""
        await self.pool.execute(
            f"DELETE from orders * WHERE order_user_id =(select user_id from users where user_telegram_id = {user_id}) "
            f"AND order_status = 'Ожидание пользователя'"
        )

    async def update_order_deliver_through(self, order_id, time_value):
        """Обновляем время, для доставки"""
        await self.pool.execute(
            f"""UPDATE orders SET order_deliver_through = {time_value} WHERE order_id = {order_id}"""
        )

    async def get_order_pass_value(self, order_id):
        """Получаем статус способо доставки"""
        return await self.pool.fetchval(
            f"SELECT order_pass_to_courier FROM orders WHERE order_id = {order_id}"
        )

    async def get_time_and_user_id(self, order_id):
        """Получаем время, за которое необходимо приготовить/доставить заказ"""
        return await self.pool.fetchrow(
            f"""SELECT order_deliver_through, user_telegram_id, location_id
FROM orders 
JOIN users on order_user_id=user_id
...
WHERE order_id = {order_id}"""
    ...
    async def get_user_tg_id(self, order_id):
        ...
        return await self.pool.fetchval(
            ...
            f'WHERE user_id = (select order_user_id from orders where order_id={order_id})'
        )
    ...
    async def reset_order_time_and_seller(self, order_id):
        ...
        await self.pool.execute(
            f"""UPDATE orders 
SET order_seller_id = null,
order_time_for_delivery = null,
order_courier_id = null,
...
WHERE order_id = {order_id}"""
    ...
    async def update_reason_for_rejection(self, order_id, reason):
        ...
        await self.pool.execute(
            f"""UPDATE orders SET 
order_reason_for_rejection = '{reason}',
order_status = 'Отклонен продавцом',
...
WHERE order_id = {order_id}"""
    ...
    async def update_reason_for_rejection_user(self, order_id, reason):
        ...
        await self.pool.execute(
            f"""UPDATE orders SET 
    order_reason_for_rejection = '{reason}',
    order_status = 'Отменен пользователем',
    order_canceled_at = now()
    WHERE order_id = {order_id}"""
        )
    ...
    async def update_bonus_review(self, order_id, review):
        """Обновляем статус заказа"""
        await self.pool.execute(
            f"""UPDATE bonus_orders SET 
    bonus_order_review = '{review}'
...
WHERE order_id = {order_id}"""
    ...
    async def take_bonus_order(self, order_id, new_seller_id):
        ...
        if bonus_seller_id == seller_id and order_status == 'Подтвержден, готовится' \
            ...
            await self.pool.execute(
                f"""UPDATE bonus_orders 
    SET bonus_order_seller_id = (SELECT seller_id FROM sellers WHERE seller_telegram_id = {new_seller_id}) 
    WHERE bonus_order_id = {order_id}"""
            )
            return True
...
select courier_name from couriers where courier_id={courier_id}""")
    ...
    async def get_courier_tg_id(self, courier_id):
        """Получаем имя курьера"""
        return await self.pool.fetchval(f"""
    select courier_telegram_id from couriers where courier_id={courier_id}""")

    async def get_seller_tg_id(self, seller_id):
        """Получаем имя курьера"""
        return await self.pool.fetchval(f"""
    select seller_telegram_id from sellers where seller_id={seller_id}""")
...
order_final_price, user_telegram_id
    ...
    async def get_user_id(self, user_tg_id):
        """Пытаемся получить пользователя из таблицы"""
        return await self.pool.fetchval(
            f'select user_id from users where user_telegram_id = {user_tg_id}'
        )
    ...
    async def get_bonus_order_info(self, user_id):
        ...
        return await self.pool.fetchrow(
            ...
            f"WHERE bonus_order_user_id = (select user_id from users where user_telegram_id={user_id})"
            f"AND bonus_order_status = 'Ожидание продавца'"
    ...
    #     async def get_bonus_bonus_little_info(self, bonus_order_id):
    #         """Получаем информацию о заказе при подтверждении продавцом"""
    #         return await self.pool.fetchrow(f"""select bonus_order_id, user_id
    # from bonus_orders
    # join users on bonus_order_user_id=user_id
    # where
    # """)
    ...
    async def set_bonus_order_status(self, order_id, status, reason=None):
        ...
        if reason:
            await self.pool.execute(
                f"UPDATE bonus_orders SET bonus_order_status = '{status}', "
                f"bonus_order_reason_for_rejection = '{reason}'"
                f"WHERE bonus_order_id = {order_id}"
            )
        else:
            await self.pool.execute(
    ...
    async def set_bonus_order_canceled_at(self, bonus_order_id):
        ...
        await self.pool.execute(
            f"UPDATE bonus_orders SET bonus_order_canceled_at = now()"
            f"WHERE bonus_order_id = {bonus_order_id}"
        )

    async def set_bonus_order_delivered_at(self, bonus_order_id):
        """Устанавливаем время отмены заказа"""
        await self.pool.execute(
            f"UPDATE bonus_orders SET bonus_order_delivered_at = now()"
            f"WHERE bonus_order_id = {bonus_order_id}"
        )
    ...
    async def set_bonus_order_taked(self, order_id):
        ...
        await self.pool.execute(
            f"UPDATE bonus_orders SET bonus_order_status = 'Подтвержден, готовится', "
            f"bonus_order_accepted_at = now()"
            f"WHERE bonus_order_id = {order_id}"
        )
    ...
    async def get_list_of_locations(self):
        ...
        return await self.pool.fetch(
            "SELECT * FROM locations ORDER BY location_metro_id"
        )
...
ORDER BY category_id;"""
    ...
    async def get_list_of_delivery_categories(self):
        """Получаем список категорий"""
        return await self.pool.fetch(
            "SELECT * FROM delivery_categories ORDER BY delivery_category_id"
        )
...
ORDER BY delivery_category_id"""
    ...
    async def delete_delivery_category_by_id(self, category_id):
        """Удаляем категорию"""
        await self.pool.execute(
            f"DELETE FROM delivery_categories WHERE delivery_category_id = {category_id}"
        )
...
WHERE admin_seller_id = {sa_id}"""
    ...
    async def delete_delivery_product_by_id(self, product_id):
        """Удаляем категорию"""
        await self.pool.execute(
            f"DELETE FROM delivery_products WHERE delivery_product_id = {product_id}"
        )
...
WHERE lc_category_id = {category_id} AND lc_location_id={location_id}"""
    ...
    async def remove_from_stock_delivery_item(self, product_id):
        """Убираем с продажи товар"""
        await self.pool.execute(
            f"""UPDATE delivery_products SET is_de_product_available=false WHERE delivery_product_id = {product_id}"""
        )
...
"""
    ...
    async def get_delivery_category_list(self):
        """Получаем список категорий"""
        return await self.pool.fetch(
            f"""SELECT * FROM delivery_categories ORDER BY delivery_category_id"""
        )
...
WHERE delivery_product_category_id={category_id}
    ...
    async def get_delivery_category_name_by_id(self, category_id):
        """Получааем название категории"""
        return await self.pool.fetchval(
            f"SELECT delivery_category_name FROM delivery_categories WHERE delivery_category_id = {category_id}"
        )
...
WHERE product_category_id = {category_id} AND is_product_available = true ORDER BY product_id"""
    ...
    async def get_delivery_products_for_remove_from_stock(self, category_id):
        ...
        return await self.pool.fetch(
            f"""SELECT delivery_product_id, delivery_product_name FROM delivery_products
    WHERE delivery_product_category_id = {category_id} AND is_de_product_available = true ORDER BY delivery_product_id"""
        )
...
ORDER BY product_category_id;"""
    ...
    async def get_orders_for_user(self, user_id):
        ...
        return await self.pool.fetch(
            f"""SELECT order_id, order_time_for_delivery, order_delivery_method, 
order_final_price, order_status
FROM orders WHERE order_user_id=(select user_id from users where user_telegram_id={user_id})
...
ORDER BY order_id"""
    ...
    async def get_bonus_orders_for_user(self, user_id):
        """Получаем активные ордеры"""
        return await self.pool.fetch(
            f"""SELECT bonus_order_id, bonus_order_quantity, bonus_order_status
    FROM bonus_orders WHERE bonus_order_user_id=(select user_id from users where user_telegram_id={user_id})
...
SET delivery_courier_status={status} WHERE delivery_courier_telegram_id = {user_id}"""
    ...
    async def get_delivery_address_for_seller_admin(self, user_id):
        ...
        return await self.pool.fetchrow(
            f"""SELECT metro.metro_name, locations.location_name, locations.location_address
from locations
...
WHERE locations.location_id = (SELECT admin_seller_location_id 
    ...
    async def get_seller_admin_location(self, user_id):
        ...
        return await self.pool.fetchval(
            f"""SELECT admin_seller_location_id 
							   FROM admin_sellers
							   WHERE admin_seller_telegram_id = {user_id}"""
        )
...
order by delivery_category_id;"""
    ...
    async def get_delivery_products(self, category_id):
        """Получаем товары из категории"""
        return await self.pool.fetch(
            f"""SELECT delivery_product_id, delivery_product_name, delivery_price
                FROM delivery_products
                WHERE delivery_product_category_id = {category_id} AND is_de_product_available = true
                ORDER BY delivery_product_id"""
        )
    ...
    async def delete_last_temp_delivery_order_by_user(self, user_id):
        ...
        await self.pool.execute(
            f"""DELETE FROM temp_delivery_orders 
WHERE temp_delivery_order_id = (SELECT temp_delivery_order_id FROM temp_delivery_orders
WHERE temp_delivery_order_user_telegram_id = {user_id} ORDER BY - temp_delivery_order_id LIMIT 1)"""
        )

    async def is_last_delivery_order(self, user_id, order_id):
        """Проверяем последний ли товар"""
        return order_id == await self.pool.fetchval(f"""SELECT temp_delivery_order_id FROM temp_delivery_orders
WHERE temp_delivery_order_user_telegram_id = {user_id} ORDER BY - temp_delivery_order_id""")
...
where local_object_id = (select order_local_object_id from orders where order_id={order_id}))""")
    ...
    async def get_seller_courier(self, order_id):
        ...
        return await self.pool.fetchrow(f"""select order_seller_id, order_courier_id
            from orders
            where order_id={order_id}""")
    ...
    async def get_bonus_order_user(self, bonus_order_id):
        ...
        return await self.pool.fetchval(f"""select user_telegram_id from users
where user_id = (select bonus_order_user_id from bonus_orders where bonus_order_id={bonus_order_id})""")
...
where  bonus_order_status in ('Выдан', 'Отменен продавцом', 'Отменен пользователем до принятия продавцом');""")
    ...
    async def get_bonus_indicators_by_location_date_period(self, location_id, first_date, last_date):
        return await self.pool.fetchrow(f"""
    select DISTINCT (SELECT
       COUNT(*) 
    FROM 
       bonus_orders
    where bonus_order_location_id = {location_id}
    and bonus_order_date >= '{first_date}'
    and bonus_order_date <= '{last_date}'
    AND bonus_order_status = 'Выдан') as completed,
...
order by order_user_id""")
    ...
    async def get_seller_info(self, seller_id):
        """Получаем статистику продавца за период"""
        return await self.pool.fetchrow(f"""
select (select count(*)
	   from orders
	   where order_seller_id={seller_id}
	   and order_status in ('Выполнен', 'Отклонен продавцом', 'Отменен пользователем', 'Отменен курьером')) as all_orders,
	   
	   (select count(*)
	   from orders
	   where order_seller_id={seller_id}
	   and order_status = 'Выполнен') as completed_orders,
	   
	   (select count(*)
	   from orders
	   where order_seller_id={seller_id}
	   and order_status = 'Отклонен продавцом') as canceled_by_seller_orders,
	   
	   (select count(*)
	   from orders
	   where order_seller_id={seller_id}
	   and order_status = 'Отменен пользователем') as canceled_by_client_orders,
	   
	   (select count(*)
	   from orders
	   where order_seller_id={seller_id}
	   and order_status = 'Отменен курьером') as canceled_by_courier_orders,
	   
	   (select count(*)
	   from bonus_orders
	   where bonus_order_seller_id={seller_id}
	   and bonus_order_status in ('Выдан', 'Отменен продавцом')) as all_bonus_orders,
	   
	   (select count(*)
	   from bonus_orders
	   where bonus_order_seller_id={seller_id}
	   and bonus_order_status = 'Выдан') as completed_bonus_orders,
	   
	   (select count(*)
	   from bonus_orders
	   where bonus_order_seller_id={seller_id}
	   and bonus_order_status = 'Отменен продавцом') as canceled_bonus_orders,
	   
	   seller_telegram_id, seller_name

from sellers
where seller_id = {seller_id}""")

    async def get_seller_info_by_date(self, seller_id, date):
        """Получаем статистику продавца за период"""
        return await self.pool.fetchrow(f"""
    select (select count(*)
    	   from orders
    ...
    async def get_seller_info_by_date_period(self, seller_id, first_date, last_date):
        """Получаем статистику продавца за период"""
        return await self.pool.fetchrow(f"""
    select (select count(*)
    	   from orders
    ...
    async def get_client_info(self, user_id, location_id):
        """Получаем статистику продавца за период"""
        return await self.pool.fetchrow(f"""
select (select count(*)
	   from orders
	   ...
	   from bonus_orders
       where bonus_order_location_id = {location_id}
	   and bonus_order_user_id={user_id}
	   and bonus_order_status in ('Выдан', 'Отменен продавцом')) as all_bonus_orders,
...
""")
    ...
    async def get_client_info_by_date(self, user_id, location_id, date):
        """Получаем статистику продавца за период"""
        return await self.pool.fetchrow(f"""
    select (select count(*)
    	   from orders
    ...
    async def get_client_info_by_date_period(self, user_id, location_id, first_date, last_date):
        """Получаем статистику продавца за период"""
        return await self.pool.fetchrow(f"""
    select (select count(*)
    	   from orders
    ...
    async def get_courier_info(self, courier_id):
        """Получаем статистику продавца за период"""
        return await self.pool.fetchrow(f"""
select (select count(*)
	   from orders
	   where order_courier_id={courier_id}
	   and order_status in ('Выполнен', 'Отклонен продавцом', 'Отменен пользователем', 'Отменен курьером')) as all_orders,
	   ...
	   courier_telegram_id, courier_name

from couriers
where courier_id = {courier_id}""")

    async def get_courier_info_by_date(self, courier_id, date):
        """Получаем статистику продавца за период"""
        return await self.pool.fetchrow(f"""
    select (select count(*)
    	   from orders
    ...
    async def get_courier_info_by_date_period(self, courier_id, first_date, last_date):
        """Получаем статистику продавца за период"""
        return await self.pool.fetchrow(f"""
    select (select count(*)
    	   from orders
...
order by order_month""")
    ...
    async def get_delivery_orders_months(self, year):
        """Получаем месяца"""
        return await self.pool.fetch(f"""select distinct extract(month from delivery_order_created_at)  as delivery_month
    from delivery_orders 
    where delivery_order_created_at is not null
    ...
    async def get_orders_days(self, year, month):
        """Получаем месяца"""
        return await self.pool.fetch(f"""select distinct order_date 
    from orders 
    where order_year = {year}
    ...
    async def get_delivery_orders_days(self, year, month):
        """Получаем месяца"""
        return await self.pool.fetch(f"""select distinct extract(day from delivery_order_created_at) as delivery_day
    from delivery_orders 
